...
@Service
public class PdfService implements com.evhub.app.constant.PDFConstant {
    ...
    private LinkedHashMap<String, Object> populateAttributesValueFromDB(LinkedHashMap<String, Object> attrMaster, JobClassifier jobClassifier ) {
        ...
        for (Map.Entry<String, Object> att : attrMaster.entrySet()) {
            if(att.getKey().contains("result")){
                attrMaster.put(att.getKey(), (jobClassifier.getResult()==null||jobClassifier.getResult().length()==0)?"NA":jobClassifier.getResult());
//            } else if(att.getKey().contains("error")){
//                attrMaster.put(att.getKey(), (jobClassifier.getErrorAndDeficiency()==null||jobClassifier.getErrorAndDeficiency().length()==0)?"NA":jobClassifier.getErrorAndDeficiency());
            } else if (att.getKey().contains("status")){
                attrMaster.put(att.getKey(),(jobClassifier.getStatus()==null||jobClassifier.getStatus().length()==0)?"NA":jobClassifier.getStatus());
            }
            else if (att.getKey().contains("description")){
                attrMaster.put(att.getKey(),(jobClassifier.getDescription()==null||jobClassifier.getDescription().length()==0)?"NA":jobClassifier.getDescription());
            } else if (att.getKey().contains("measuredValue")){
                attrMaster.put(att.getKey(),(jobClassifier.getMeasuredValue()==null||jobClassifier.getMeasuredValue().length()==0)?"NA":jobClassifier.getMeasuredValue());
            }
        }
